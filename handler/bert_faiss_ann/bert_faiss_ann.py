...
def create_vectors_and_faiss_index(data, model, tokenizer):
    ...
    # Số lượng cluster (tối ưu hóa hiệu suất, thường chọn sqrt(n))
    nlist = int(np.sqrt(len(product_vectors)))

    # Tạo index FAISS với IndexIVFFlat
    quantizer = faiss.IndexFlatIP(d)  # Sử dụng inner product để làm quantizer
